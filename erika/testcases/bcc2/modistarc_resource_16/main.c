...
TASK(Task1)
{
  ...
  StatusType ret2 = ReleaseResource(RES_SCHEDULER); // Preemption
...
Expect:
Resource is released and running task’s priority is reset. Ready task with highest priority is executed (Rescheduling). 
...
int main(void)
{
  ...
  EE_assert_summarize("BCC2 Modistarc Resource Management (16)", result, ASSERTIONS);
